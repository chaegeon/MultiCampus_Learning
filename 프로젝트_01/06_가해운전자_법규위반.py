...
#axes1.figure(figsize=(20,10))
axes1.plot(1, 2, 1)
#explode : array-like, default None
wedgeprops = {'width':0.7, 'edgecolor':'w', 'linewidth':1}
explodes =(0.1 ,0,0,0,0,0,0,0)
axes1.pie(accident, labels = rawData['위반유형'] ,autopct='%.1f%%',startangle=150, counterclock=False, wedgeprops=wedgeprops, explode=explodes)
plt.show()

#axes2.figure(figsize=(8,8))
axes2.plot(1,2,2)
fig.set_facecolor('white')
ax1 = fig.add_subplot()
# plt.ylim([0, 900])
...
ax2 = ax1.twinx() 
axes2 = ax2.plot(xtick_label_position, death, color='r', linestyle='--', marker='o', label = '사망자 수')
ax2.tick_params(axis='y', labelcolor='r')
ax2.set_ylabel('사망자 수')
for i in range( len(a_type)):
  plt.text( i+0.1, death[i]+0.5, f'{death[i]}', size = 14, color='r')
...
explodes =(0.1 ,0,0,0,0,0,0,0)
plt.pie(accident, labels = rawData['위반유형'] ,autopct='%.1f%%',startangle=150, counterclock=False, wedgeprops=wedgeprops, explode=explodes)
plt.title('노인보행사고 중 가해운전자 법규위반 유형 비율')
plt.show()
